@echo off
REM Fix nginx configuration with correct IP addresses (Version 2)
...
REM Update nginx.conf with correct IPs - handle both DNS names and existing IPs
...
REM Also replace any existing IP addresses with the new ones
powershell -Command "(Get-Content nginx.conf) -replace 'http://172\.18\.0\.[0-9]+:8000/', 'http://!BACKEND_IP!:8000/' | Set-Content nginx.conf"
powershell -Command "(Get-Content nginx.conf) -replace 'http://172\.18\.0\.[0-9]+:3000', 'http://!FRONTEND_IP!:3000' | Set-Content nginx.conf"
